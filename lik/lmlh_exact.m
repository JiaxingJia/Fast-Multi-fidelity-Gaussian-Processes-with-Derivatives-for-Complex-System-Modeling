...
function [val, dval] = lmlh_exact(cov, y, hyp, lambda)
global GP_grad_info
...
if isfield(hyp, 'lik')
    if length(hyp.lik) == 2 % Use one sigma for the values, one for the derivatives
        ...
        sig = [sigma1*ones(1,n), sigma2*ones(1,n*d)]; GP_grad_info.sig = sig;
    ...
    [L, p] = chol(K + lambda*eye(nd), 'lower'); GP_grad_info.L = L;
end
...
F = GP_grad_info.F;
belta0 = (F'*(L\(L'\y(:))))/(F'*(L\(L'\F))); GP_grad_info.belta0 = belta0;   % Most important part !!!
alpha = L'\(L\(y(:) - belta0*F));
term1 = (y(:) - belta0*F)'*alpha; % This is y'*(K\y)
term2 = 2*sum(log(diag(L))); % This is log(det(K))
val = 0.5*(term1 + term2 + nd*log(2*pi)); % Negative ML formula
